...
char dict[][GRID_SIZE] = {"PRACTICA","PROGRAMACION"/*};*/,"INTERNET","DIFERENCIAL",};
void print_cmatrix(char w[][GRID_SIZE]);
void setwrd(char w[][GRID_SIZE],char dictionary[][GRID_SIZE]);
int wsize(char w[]);
void sopa(){
    int i,j;
    //Rango de caracteres en el que se genera la sopa de letras
    int range = 'z'-'a';
    ...
    //Se agregan las palabras del diccionario a la sopa de letras
    setwrd(w,dict);
